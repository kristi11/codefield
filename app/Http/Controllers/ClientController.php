...
class ClientController extends Controller
{
    ...
    public function userProfilePhotos($slug){
        ...
        public function searchUserProfile($id,$slug){
        $title = 'Profile';
        $userData = User::find($id)->where('slug',$slug)->first();
        $data = Gallery::where('user_id',$userData)->latest()->get();
        dd($data);
        // $user = User::where('slug',$slug)->first();
        //     if (count($data)<=0) {
        //         $p = "User hasn't added any photos yet";
        //         return view('client.partials.uPhotos',compact('title','data','p','user'));
        //     }
        //     else
        //     {
        //         return view('client.partials.uPhotos',compact('title','data','user'));
        //     }
