	...
	<div class="col-xs-12 text-right">
		<form  method="POST" action="{{ url('delete_all') }}">
			...
			<button 
				...
				type="submit" 
				onclick="return confirm('Empty Trash?');">
				Empty trash
			</button>
		</form>
	</div>
		...
		<div class="card card-stats">
			...
			<div class="card-footer">
				<button hidden="">
					<form 
						method="POST" 
						action="{{ url('/admin/deleted_projects/'.$project->id) }}">
							{{ csrf_field() }}
						<button 
							class="btn btn-success btn-simple btn-xs" 
							type="submit" 
...
<div class="col-xs-6 col-sm-3 col-md-6 col-lg-3 text-center">
	<div class="card fe_pr">
		<div class="pr_img">
			...
			src="{{ Storage::url('storage/mobile_photos/'.$image->gallery_image) }}">
		</div>
		<div class="card-content">					
			<button hidden="">
				<form 
					method="POST" 
					action="{{ url('/admin/trash/'.$image->id) }}">
					{{ csrf_field() }}
					<button 
						class="btn btn-success btn-simple btn-xs" 
						type="submit" 
